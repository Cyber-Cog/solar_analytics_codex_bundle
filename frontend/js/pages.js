...
window.DashboardPage = ({ plantId, dateFrom, dateTo, onNavigate }) => {
  ...
  return h('div', { style: { position: 'relative' } },
    ...
    noData && h('div', {
      ...
      style: {
        background: 'linear-gradient(135deg, rgba(251, 146, 60, 0.18), rgba(234, 88, 12, 0.08))',
    ...
    h(Card, { title:'Performance KPIs', style:{marginBottom:16} },
      h('div', { className:'kpi-grid kpi-grid--performance', style:{gridTemplateColumns:'repeat(3, 1fr)'} },
        h(KpiCard, { variant:'performance', subVariant:'today-energy', label:'Energy Export',  value: energyExportKpi.value, unit: energyExportKpi.unit, color:'#c2410c', icon: 'Leaf' }),
        h(KpiCard, { variant:'performance', subVariant:'total-power',  label:'Peak Power',     value:fmtNum(kpis?.peak_power_kw),     unit:'kW', color:'#ea580c', icon: 'Zap' }),
        h(KpiCard, { variant:'performance', subVariant:'current',      label:'Active Power',   value:fmtNum(kpis?.active_power_kw),   unit:'kW', color:'#d97706', icon: 'Activity' }),
        h(KpiCard, { variant:'performance', subVariant:'total-energy', label:'Perf. Ratio',    value:fmtNum(kpis?.performance_ratio), unit:'%',  color:'#b45309', icon: 'TrendingUp' }),
        h(KpiCard, { variant:'performance', subVariant:'voltage',      label:'PLF',            value:fmtNum(kpis?.plant_load_factor), unit:'%',  color:'#9a3412', icon: 'Gauge' }),
        h(KpiCard, { variant:'performance', subVariant:'co2-avoided',  label:'Net Generation', value: netGenKpi.value, unit: netGenKpi.unit, color:'#f97316', icon: 'Cloud' }),
      ),
    ),
    ...
    h('div', { style:{display:'grid', gridTemplateColumns:'2fr 1fr', gap:16, marginBottom:16, alignItems:'stretch'} },
      ...
      h(Card, { title:'WMS - Weather Sensors', style:{ minHeight: 480, display:'flex', flexDirection:'column' } },
        h('div', { className:'kpi-grid kpi-grid--performance', style:{ gridTemplateColumns:'repeat(2, 1fr)', flex:1 } },
          ...
          h(KpiCard, { variant:'performance', subVariant:'wms-irr-horz', label:'Irradiance (Horizontal)', value:fmtNum(wms?.irradiance_horizontal, 1), unit:'W/m²', color:'#c2410c', icon:'Compass' }),
          h(KpiCard, { variant:'performance', subVariant:'wms-ambient', label:'Amb. Temp', value:fmtNum(wms?.ambient_temp), unit:'°C', color:'#9a3412', icon:'Thermometer' }),
          h(KpiCard, { variant:'performance', subVariant:'wms-module', label:'Mod. Temp', value:fmtNum(wms?.module_temp), unit:'°C', color:'#7c2d12', icon:'Flame' }),
          h(KpiCard, { variant:'performance', subVariant:'wms-wind', label:'Wind Speed', value:fmtNum(wms?.wind_speed), unit:'m/s', color:'#a16207', icon:'Wind' }),
          h(KpiCard, { variant:'performance', subVariant:'wms-rain', label:'Rain gauge', value:fmtNum(wms?.rainfall_mm, 2), unit:'mm', color:'#b45309', icon:'CloudRain' }),
        ),
      ),
    ),
